    ...
    public class SwaggerControllerOrder<T>
    {
        ...
        public SwaggerControllerOrder(IEnumerable<Type> controllers)
        {
            ...
            orders = new Dictionary<string, uint>(
                controllers.Where(c => c.GetCustomAttributes<SortSwaggerPathsByMethod>().Any())
                .Select(c => new { Name = ResolveControllerName(c.Name), c.GetCustomAttribute<SortSwaggerPathsByMethod>().Order })
